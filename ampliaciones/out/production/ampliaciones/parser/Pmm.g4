...
expression returns [Expresion ast]: ID {$ast = new Variable($ID.getLine(), $ID.getCharPositionInLine() + 1, $ID.text);}
            ...
            | e=expression op=('--'|'++') {$ast = new PostArithmetic($e.start.getLine(), $e.start.getCharPositionInLine() + 1, $e.ast, $op.text);}
            | op=('--'|'++') e=expression  {$ast = new PreArithmetic($op.getLine(), $op.getCharPositionInLine() + 1, $e.ast, $op.text);}
