...
program returns [Program ast]: {List<Definition> defs = new ArrayList<Definition>();}
...
/*Reglas Sintactico*/
/*Programa */
main returns [FunctionDefinition ast]: d='def' m='main' function_type '{' function_body '}'  {$ast = new FunctionDefinition($d.getLine(), $d.getCharPositionInLine() + 1, $m.text, $function_type.ast, $function_body.ast);}
                ;

function_definition returns [FunctionDefinition ast]: d='def' ID function_type'{' function_body '}' {$ast = new FunctionDefinition($d.getLine(), $d.getCharPositionInLine() + 1, $ID.text, $function_type.ast, $function_body.ast);}
                    ;
...
parameters returns [List<VariableDefinition> ast = new ArrayList<VariableDefinition>()]: (v1=variable_definition {$ast.add($v1.ast)}(',' v2=variable_definition{$ast.add($v2.ast)})*)?
            ;

variable_definition returns [List<VariableDefinition> ast = new ArrayList<VariableDefinition>()]: id1=ID {$ast.add(new VariableDefinition($id1.getLine(), $id1.getCharPositionInColumn()+1, $id1.text, null))}
                            (',' id2=ID{$ast.add(new VariableDefinition($id2.getLine(), $id2.getCharPositionInColumn()+1, $id2.text, null))})* ':' type {for (VariableDefinition i: $ast) {i.setType($type.ast);}}
		            ;

function_body returns [List<Statement> ast = new ArrayList<Statement>()]:
                | (variable_definition ';' {$ast.add($variable_definition.ast)})+ statements{$ast.add($statements.ast)}
                | (variable_definition ';' {$ast.add($variable_definition.ast)})+
                | statements{$ast.add($statements.ast)}
                ;
...
simple_type returns [Type ast]: i='int' {$ast = new IntType(+$.getLine(), $i.getCharPositionInLine() + 1);}
...
struct_fields returns [List<StructureField> ast = new ArrayList<StructureField>()]: id1=ID {$ast.add(new VariableDefinition($id1.getLine(), $id1.getCharPositionInColumn()+1, $id1.text, null))}
                    (',' id2=ID{$ast.add(new VariableDefinition($id2.getLine(), $id2.getCharPositionInColumn()+1, $id2.text, null))})* ':' type {for (VariableDefinition i: $ast) {i.setType($type.ast);}}
            ;

/*Statement*/
statement returns [List<Statement> ast = new ArrayList<Statement>()]: assigment {$ast.add($assigment.ast);}
...
ifElse returns [IfElse ast]: op='if' e=expressions ':' s1=statement {$ast = new IfElse($op.getLine(), $op.getCharPositionInLine() + 1, $e.ast, $s1.ast);}
        | op='if' e=expressions ':' s1=statement 'else' '{' ss2=statements '}' {$ast = new IfElse($op.getLine(), $op.getCharPositionInLine() + 1, $e.ast, $s1.ast, $ss2.ast);}
        | op='if' e=expressions ':' s1=statement 'else' s2=statement {$ast = new IfElse($op.getLine(), $op.getCharPositionInLine() + 1, $e.ast, $s1.ast, $s2.ast);}
        | op='if' e=expressions ':' '{' ss1=statements '}' {$ast = new IfElse($op.getLine(), $op.getCharPositionInLine() + 1, $e.ast, $ss1.ast);}
        | op='if' e=expressions ':' '{' ss1=statements '}' 'else' s2=statement {$ast = new IlElse($op.getLine(), $op.getCharPositionInLine() + 1, $e.ast, $ss1.ast, $s2.ast );}
        | op='if' e=expressions ':' '{' ss1=statements '}' 'else' '{' ss2=statements '}' {$ast = new IlElse($op.getLine(), $op.getCharPositionInLine() + 1, $e.ast, $ss1.ast, $ss2.ast );}
...
/*Expression */
expression returns [Expression ast]: ID {$ast = new Variable($ID.text)}
            ...
            | e1=expression '.' ID {$ast = new FieldAccess($e1.start.getLine(), $e1.start.getCharPositionInLine() + 1, $expression.ast, $ID.text);}
            | e1=expression op=('*'|'/'|'%') e2=expression {$ast = new Arithmetic($e1.start.getLine(), $e1.start.getCharPositionLine() + 1, $e1.ast, $e2.ast, $op.text);}
            | e1=expression op=('+'|'-') e2=expression {$ast = new Arithmetic($e1.start.getLine(), $e1.start.getCharPositionLine() + 1, $e1.ast, $e2.ast, $op.text);}
            | e1=expression ('>'|'>='|'<'|'<='|'!='|'==') e2=expression {$ast = new Arithmetic($e1.start.getLine(), $e1.start.getCharPositionLine() + 1, $e1.ast, $e2.ast, $op.text);}
            | e1=expression ('&&'|'||') e2=expression {$ast = new Arithmetic($e1.start.getLine(), $e1.start.getCharPositionLine() + 1, $e1.ast, $e2.ast, $op.text);}
            | op='('simple_type')' expression {$ast = new Cast($op.getLine(), $op.getCharPositionLine() + 1, $simple_type.ast, $expression.ast);}
            | op='-' expression {$ast = new UnaryMinus($op.getLine(), $op.getCharPositionLine() + 1, $expression.ast);}
            | invokeFunction {$ast = $invokeFunction.ast;}
            ;

invokeFunction returns [InvokeFunction ast]: variable'(' expressions ')' {$ast = new InvokeFunction($variable.start.getLine(), $variable.start.getCharPositionLine() + 1, $expressions.ast, $variable.ast);}
            ;
